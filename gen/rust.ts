...
function custom_serialize_deserialize_type(t: Type, derives: string[]): string {
    let serde_options: string[] = [];
...
function render_struct_field(f: Field, derives: string[]): string {
    return `${custom_serialize_deserialize_type(f.type, derives)}pub ${f.name}: ${render_type(f.type)}`;
}

function render_union_variant_field(f: Field, derives: string[]): string {
    return `${custom_serialize_deserialize_type(f.type, derives)}${f.name}: ${render_type(f.type)}`;
}

function render_tuple_union_variant_field(t: Type, derives: string[]): string {
    return `${custom_serialize_deserialize_type(t, derives)}${render_type(t)}`;
}

function gen_struct_union_variant(variant: Struct, derives: string[]): string {
...
function gen_tuple_union_variant(variant: { name: string, type: Type; }, derives: string[]): string {
    return `${variant.name} (${render_tuple_union_variant_field(variant.type, derives)})`;
}

export function gen_type_declaration(decl: TypeDeclaration, derives: string[]): string {
    let r = "";
...
export let DOUBLE_OPTION_SERIALIZE_DESERIALIZE: string =
...
export let NULLABLE_SERIALIZE_DESERIALIZE = `
#[derive(Debug)]
